...
namespace MRL.Commons.Tree
{
        ...
        where TField : struct, IEquatable<TField>, IFormattable
    {
        ...
        /// </remarks>
        public KdTreeNode<TValue> Add(TValue value, KdTreeNode<TValue> parent)
        {
            if (value == null)
                throw new ArgumentNullException("value");

            return Add(value, parent, this.root, 0);
        }

        private KdTreeNode<TValue> Add(TValue value, KdTreeNode<TValue> parent, KdTreeNode<TValue> node, int depth)
        {
            if (node == null)
            {
            ...
            else
            {
                ...
                if (comparison <= 0)
                {
                    node.LeftChild = Add(value, parent, node.LeftChild, depth + 1);
                }
                else
                {
                    node.RightChild = Add(value, parent, node.RightChild, depth + 1);
                }
            }
        ...
        private TValue FindMinimum(KdTreeNode<TValue> node, int splittingDimension, int depth)
        {
            ...
            else
            {
                ...
                if (leftMinValue != null && fieldComparer.Compare(
                    this.locationGetter(leftMinValue)[splittingDimension], nodeLocation[splittingDimension]) < 0)
                    return leftMinValue;
                else if (rightMinValue != null && fieldComparer.Compare(
                    this.locationGetter(rightMinValue)[splittingDimension], nodeLocation[splittingDimension]) < 0)
