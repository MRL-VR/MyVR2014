...
namespace MRL.CustomMath.Base
{
    ...
    public class Vector
    {
        ...
        {
            string[] tmp = vals.Split(",".ToCharArray());
            _length = tmp.Length;
            _v = new double[_length];

            for (int i = 0; i < _length; i++)
                this[i] = double.Parse(tmp[i]);
        }
        ...
        {
            get { return _v.Select(x => (float)x).ToArray(); }
            set
            {
                if (value == null || value.Length != Length)
                    throw new Exception("Invalid FloatArray for Vector");
                _v = value.Select(x => (double)x).ToArray();
            }
        }
