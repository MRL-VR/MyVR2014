...
namespace MRL.Exploration.ScanMatchers.Base
{
    public abstract class IcpScanMatcher : IScanMatchers
    {
        ...
        {
            double fov = scan.RangeScanner.FieldOfView;
            double res = scan.RangeScanner.Resolution;
        ...
        MatchResult IScanMatchers.Match(ScanObservation scan1, ScanObservation scan2, Pose2D seed)
        {
            ...
            if ((scan1.RangeScanner.FieldOfView == scan2.RangeScanner.FieldOfView) && (scan1.RangeScanner.Resolution == scan2.RangeScanner.Resolution))
            {
            ...
            double dangle = scan1.RangeScanner.Resolution;
        ...
        protected virtual bool[] ToFilter(ScanObservation scan)
        {
            ...
            for (int i = 0; i <= len; i++)
            {
                filter[i] = (scan.RangeScanner.Range[i] < scan.RangeScanner.MinRange) || (scan.RangeScanner.Range[i] > scan.RangeScanner.MaxRange);
            }
            return filter;
        }
        ...
        public static Pose2D AddPoses(Pose2D t1, Pose2D t2)
        {
            Pose2D t_ret = new Pose2D();
